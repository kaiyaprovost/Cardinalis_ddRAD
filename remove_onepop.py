...
##for i in lines:
##    if i in onepop:
##        print >>cardout_1pop, "locus"+str(counti), lines[i]
##    if i not in onepop:
##        print >>cardout_2pop, "locus"+str(counti), lines[i]
##    counti = counti + 1
...
for i in range(len(lines)):
    if locusCount in onepop:
        if lines[i][0]== ">":
            print >>cardout_1pop, lines[i].strip()
        else:
            locusCount = locusCount + 1
            print >>cardout_1pop, "//"
    else:
        if lines[i][0]== ">":
            print >>cardout_2pop, lines[i].strip()
        else:
            locusCount = locusCount + 1
            print >>cardout_2pop, "//"
